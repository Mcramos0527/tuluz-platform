...
const FROM = 'Tu Luz <hola@tuluz.org>'

export async function sendWelcomeEmail(to: string, name: string) {
  return getResend().emails.send({
    from: FROM,
    to,
    subject: '¡Bienvenida a Tu Luz! 🌱',
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#1a3a2a">
        <div style="background:#2d6a4f;padding:32px;border-radius:12px 12px 0 0;text-align:center">
          <h1 style="color:#ffffff;margin:0;font-size:28px">Bienvenida a Tu Luz</h1>
        ...
        <div style="background:#f8fffe;padding:32px;border-radius:0 0 12px 12px;border:1px solid #d8f3dc">
          ...
          <p>En Tu Luz te acompañamos a construir un negocio con propósito, certificar tu impacto positivo y conectar con empresas que valoran lo que tú creates.</p>
          ...
          <p style="color:#74c69d;font-size:13px">Con cariño, el equipo de Tu Luz 🌿</p>
        </div>
      </div>
    `,
  })
    ...
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#1a3a2a">
        ...
        <div style="background:#f8fffe;padding:32px;border-radius:0 0 12px 12px;border:1px solid #d8f3dc">
          ...
          <p>¡Felicitaciones! Has obtenido la insignia <strong>${label}</strong> de la Asociación Tu Luz.</p>
          <div style="text-align:center;margin:24px 0">
            <img src="${badgeUrl}" alt="Insignia Tu Luz" style="max-width:300px;border-radius:8px" />
          </div>
          <p>Puedes verificar tu insignia en: <a href="${verificationUrl}">${verificationUrl}</a></p>
          <p style="color:#74c69d;font-size:13px">El equipo de Tu Luz 🌿</p>
        </div>
      </div>
    `,
  })
...
export async function sendProfileApprovedEmail(to: string, name: string) {
  return getResend().emails.send({
    from: FROM,
    to,
    subject: '¡Tu perfil ha sido aprobado en Tu Luz!',
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto;color:#1a3a2a">
        <div style="background:#2d6a4f;padding:32px;border-radius:12px 12px 0 0;text-align:center">
        ...
        <div style="background:#f8fffe;padding:32px;border-radius:0 0 12px 12px;border:1px solid #d8f3dc">
          <p>Hola <strong>${name}</strong>,</p>
          <p>Tu perfil de emprendedora ha sido aprobado y ya es visible en el Marketplace de Tu Luz.</p>
